...
public class Model {
	...
	public Page doPage(int type,int currentPage,int pageSize, int flag,String orderPrice){
		return bd.doPage(type,currentPage, pageSize,flag,orderPrice);
	}
	//根据用户输入的关键字搜索相关商品
	...
	public List searchGoodsByConditions(int superTypeId,String searchMethod){
		return bd.searchGoodsByConditions(superTypeId, searchMethod);//djm
	}
	/**
	 *根据用户选择的搜索条件搜索相关商品
	 * @param superTypeId 所属大类的ID
	...
	public Page doPageByConditons(int superTypeId,String searchMethod){
		return bd.doPageByConditons(superTypeId, searchMethod);
	}
	/**
	 * 后台*/
	...
	public boolean updateSuperType(SuperType type){
		return superTypeDao.updateSuperType(type);
	}	
	
	public boolean deleteSuperType(int[] ids) {
		return superTypeDao.deleteSuperType(ids);
	}
	...
	public boolean unFreezeOrder(int[] orderids) {
		return adminOrderDao.unFreezeOrder(orderids);
	}
	
	public boolean confirmOrder(int orderid) {
		return adminOrderDao.confirmOrder(orderid);
	}
	//根据当前用户的用户名查订单
