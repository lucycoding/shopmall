...
public class GoodsDaoImpl implements GoodsDao {
	...
	public boolean updateGoods(Goods Goods){
		DbUtil daoUtil = null;
		PreparedStatement ps = null;
		String sql = "update tb_goods set goodsName=?,goodsNum=?,introduce=?,ISBN=?,author=?,superTypeId=?,publisher=?,price=?,nowPrice=?," +
		...
		try {
            ...
            ps.setString(7, Goods.getPublisher());
            ps.setFloat(8, Goods.getPrice());
            ps.setFloat(9, Goods.getNowPrice());
            ps.setString(10, Goods.getProduceDate());
            ps.setInt(11, Goods.getHostGoods());
	...
	public boolean addGoods(Goods Goods){
		...
		try {
			...
			pstmt.setString(3, Goods.getISBN());
			pstmt.setString(4, Goods.getIntroduce());
			pstmt.setFloat(5, Goods.getPrice());
			pstmt.setFloat(6, Goods.getNowPrice());
			pstmt.setString(7, Goods.getPicture());
			pstmt.setString(8, Goods.getProduceDate());
			pstmt.setString(9, Goods.getPublisher());
			pstmt.setString(10, Goods.getAuthor());
			pstmt.setInt(11, Goods.getNewGoods());
			pstmt.setInt(12, Goods.getSaleGoods());
			pstmt.setInt(13, Goods.getHostGoods());
			pstmt.setInt(14, Goods.getGoodsNum());
	 ...
	 * @return Page
	 */
	public Page doPage(int type,int currentPage,int pageSize,int flag, String orderPrice){
		Page page = new Page();
		int totalNum = 0;
		if(flag == -1) {
		...
		String order="bookId asc";
		if(orderPrice!=null && !"".equals(orderPrice)){
			order="nowPrice "+orderPrice;
		}
		...
		if(flag == -1) {
			if(type==0){//显示所有商品
				sql = "select * from tb_goods order by "+order+" limit "+currentPage+","+pageSize;
			}
			if(type==1) {
				//热卖商品
				sql = "select * from tb_goods where hostGoods=1 order by "+order+" limit "+currentPage+","+pageSize;
			}
			if(type==2){
				//新到商品
				sql = "select * from tb_goods where newGoods=1 order by "+order+" limit "+currentPage+","+pageSize;
			}
			if(type==3){
				//打折商品
				sql = "select * from tb_goods where saleGoods=1 order by "+order+" limit "+currentPage+","+pageSize;
			}
			if(type==4){
				//特别推荐
				sql = "select * from tb_goods where specialGoods=1 order by "+order+" limit "+currentPage+","+pageSize;
			}
		} else {
			//分类商品
			sql = "select * from tb_goods where superTypeId="+flag+" order by "+order+" limit "+currentPage+","+pageSize;
		}
	...
	public Page doPage(String keywords,int currentPage,int pageSize){
		...
		ResultSet rs = null;
		DbUtil dbUtil = null;
		String sql = "select * from tb_goods where (goodsName LIKE ?) OR (ISBN LIKE ?) OR (introduce LIKE ?) OR (publisher LIKE ?) OR (author LIKE ?) limit "+currentPage+","+pageSize;
		try {
			dbUtil = new DbUtil();
			pstmt = dbUtil.getCon().prepareStatement(sql);
	...
	public List searchGoodsByConditions(int superTypeId,String searchMethod){
		...
		DbUtil dbUtil = null;
		String sql = null;
		sql = "select * from tb_goods where superTypeId=? and "+str[0]+" like ?";
		try {
			dbUtil = new DbUtil();
			pstmt = dbUtil.getCon().prepareStatement(sql);
	 ...
	 */
	public Page doPageByConditons(int superTypeId,String searchMethod){
		Page page = new Page();
		String str[] = searchMethod.split("=");
		int totalNum = searchGoodsByConditions(superTypeId,searchMethod).size();
		List pageList = new ArrayList();
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		DbUtil dbUtil = null;
