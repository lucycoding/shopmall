...
public class AdminGoodsDaoImpl implements AdminGoodsDao{
	...
	public boolean addGoods(Goods Goods) {
		...
		try {
			...
			ps.setString(3, Goods.getISBN());
			ps.setString(4, Goods.getIntroduce());
			ps.setFloat(5, Goods.getPrice());
			ps.setFloat(6, Goods.getNowPrice());
			ps.setString(7, Goods.getPicture());
			ps.setString(8, Goods.getProduceDate());
			ps.setString(9, Goods.getPublisher());
			ps.setString(10, Goods.getAuthor());
			...
			ps.setInt(15, Goods.getGoodsNum());
	...
	public GoodsPager searchGoods(String GoodsName) {
		...
		try {
			daoUtil = new DbUtil();
			sql = "select * from tb_goods where (goodsName LIKE ?) OR (ISBN LIKE ?) OR (introduce LIKE ?) OR (publisher LIKE ?) OR (author LIKE ?)";
			ps = daoUtil.getCon().prepareStatement(sql);
			ps.setString(1, "%"+GoodsName+"%");
