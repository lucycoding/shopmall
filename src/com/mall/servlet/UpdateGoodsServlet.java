...
public class UpdateGoodsServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
	    ...
	    if(model.updateGoods(Goods)){//修改成功
	    	...
	    	session.setAttribute("updateMessage","<script>bootbox.alert('修改成功!');location.href='getGoodsPagerServlet';</script>");
	    	request.getRequestDispatcher("Admin/pages/updateGoods.jsp").forward(request, response);
	    }else{//修改失败
	    	session.setAttribute("updateMessage","<script>bootbox.alert('修改失败!');</script>");
	    	request.getRequestDispatcher("Admin/pages/updateGoods.jsp").forward(request, response);
